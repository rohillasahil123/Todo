import React, { useEffect } from 'react'
import Button from '../components/Button'
import { useState } from 'react'
import axios from "axios"
import toast from 'react-hot-toast'
import { Navigate, useNavigate, useParams } from 'react-router-dom'

const Update_Page = () => {
  const [obj, setObj] = useState({
    title: "",
    description: ""
  })

  const { id } = useParams()
  const navigate = useNavigate()

  const onHandleChange = (e) => {
    setObj({ ...obj, [e.target.name]: e.target.value })
  }


  useEffect(() => {
    UpdateTask(id)
  }, [id])

  const UpdateTask = async (id) => {
    try {
      const response = await axios.get(`http://localhost:5000/update/todo/${id}`)
      const result = response.data.todo
      setObj({
        title: result.title,
        description: result.description
      })    } catch (error) {
      console.log(error)
    }}



    const handleUpdateTask =async(id)=>{
      try {
        const updatedResult = await axios.put(`http://localhost:5000/update-task/${id}`,obj)
        if(updatedResult.status){
          toast.success("Updted success")
          navigate("/list")
        }
        console.log(updatedResult.status , "true")
      } catch (error) {
        
      }

    }


  return (
    <div className='w-full h-[90vh] flex justify-center items-center'>
     <div className='h-[40vh] sm:h-[60vh] w-[70%] sm:w-[34%] shadow-2xl border flex flex-col     '>

        <h1 className='text-center font-bold text-2xl mb-6'>Add New Task</h1>

        <div className='flex flex-col items-center w-full gap-4'>

          <div className="w-[70%] flex flex-col">
            <label htmlFor="text" className='mb-1 font-medium text-gray-700'>
              Task Title
            </label>
            <input
              onChange={onHandleChange}
              type="text"
              id="text"
              name='title'
              value={obj.title}
              className='border px-3 py-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-400'
            />
          </div>

          <div className="w-[70%] flex flex-col">
            <label htmlFor="des" className='mb-1 font-medium text-gray-700'>
              Description
            </label>
            <textarea
              onChange={onHandleChange}
              value={obj.description}
              id="description"
              name='description'
              className='border px-3 py-2 rounded h-28 resize-none focus:outline-none focus:ring-2 focus:ring-blue-400'>
            </textarea>
          </div>
          <Button text="Update task" color='bg-blue-700' textcolor='text-white ' hoverColor='bg-blue-900 ' onClick={()=>handleUpdateTask(id)} />
        </div>
      </div>
    </div>
  )
}

export default Update_Page
